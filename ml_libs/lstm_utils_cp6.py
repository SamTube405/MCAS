...
'''        
def getFeatureTargetSequenceLSTM(x,xnarrative,target,features_local=[], features_global=[], time_in=0, time_out=0, narrative_list=[], daily_pred=True):
    '''
    ...
    try:
        ...
        if daily_pred:
            ...
            target_variable=target.loc[x:x+timedelta(weeks=time_out-1)][xnarrative].values
    except:
    ...
    for i,df in enumerate(features_local):
        if i == 0:
            if daily_pred:
                ...
                features=df.loc[x-timedelta(weeks=time_in):x-timedelta(weeks=1)][[xnarrative]].values
        else:
            if daily_pred:
                ...
                features_=df.loc[x-timedelta(weeks=time_in):x-timedelta(weeks=1)][[xnarrative]].values
                features=np.concatenate([features,features_], axis=1)
    ...
    for i, df in enumerate(features_global):
        if type(features)==int and i==0:
            if daily_pred:
                ...
                features=df.loc[x-timedelta(weeks=time_in):x-timedelta(weeks=1)].values
        else:
            if daily_pred:
                features_=df.loc[x-timedelta(days=time_in):x-timedelta(days=1)].values
            else:
                features_=df.loc[x-timedelta(weeks=time_in):x-timedelta(weeks=1)].values
            if features_.ndim<2:
...
'''
def getSimulationSequenceLSTM(x,xnarrative,target,features_local=[], features_global=[], time_in=0, time_out=0, narrative_list=[], daily_pred=True):
    '''
            ...
            try:
                ...
                try:
                    ...
                    if daily_pred:
                        ...
                        features=df.loc[x-timedelta(weeks=time_in):][[xnarrative]].values
                    pad = time_in - features.shape[0]
                    features=np.expand_dims(np.append(features, np.array([np.NaN]*pad)), axis=1)
                except:  
                    ### pad with np.NaNs
            ...
            try:
                ...
                try:
                    ...
                    if daily_pred:
                        ...
                        features_=df.loc[x-timedelta(weeks=time_in):][[xnarrative]].values
                    pad = time_in - features_.shape[0]
                    features_=np.expand_dims(np.append(features_, np.array([np.NaN]*pad)), axis=1)
                except:    
...
def data_prepare_LSTM(sim_start_date, sim_end_date, target, features_local=[],
                    features_global=[], time_in=0, time_out=0, narrative_list=[], daily_pred=True):
    '''
        ...
        if daily_pred:
            for x in datespan(sim_start_date,sim_end_date-timedelta(days=time_out-2),delta=timedelta(days=1)):
                features,target_variable=getFeatureTargetSequenceLSTM(x,Tnarrative, target, narrative_list=narrative_list,
                                                                  ...
                                                                  time_in=time_in, time_out=time_out, daily_pred=daily_pred)

                data_X.append(features)
                data_y.append(target_variable)
        else:
            for x in datespan(sim_start_date,sim_end_date-timedelta(weeks=time_out-1),delta=timedelta(weeks=1)):
                features,target_variable=getFeatureTargetSequenceLSTM(x,Tnarrative, target, narrative_list=narrative_list,
                                                                  features_local=features_local, features_global=features_global,
                                                                  time_in=time_in, time_out=time_out, daily_pred=daily_pred)

                data_X.append(features)
                data_y.append(target_variable.astype("float"))
...
'''
def getReplayBaselinePredictions(window_start_date, window_end_date,target, narrative_list=[], daily_pred=True):
  
    sim_days = window_end_date - window_start_date
    sim_days = sim_days.days + 1
    
    if daily_pred==False:
        sim_days = round(sim_days/7,0)
    ...
    if daily_pred:
        ...
        baseline_window_end_date=(window_start_date-timedelta(weeks=1)).strftime("%Y-%m-%d")
        baseline_window_start_date=(window_start_date-timedelta(weeks=sim_days)).strftime("%Y-%m-%d")
    baseline_target=target[baseline_window_start_date:baseline_window_end_date]
    
    narrative_baseline_data={}
    for Tnarrative in narrative_list:
        Tnarrative_ = Tnarrative.replace("informationID_","") 
        y_hat=baseline_target[Tnarrative].values
        narrative_baseline_data.setdefault(Tnarrative_,y_hat)
    return narrative_baseline_data

'''
Get Replay Baseline predictions
'''
def getSamplingBaselinePredictions(window_start_date, window_end_date,target, narrative_list=[], daily_pred=True):
    sim_days = window_end_date - window_start_date
    sim_days = sim_days.days + 1
    ...
    if daily_pred:
        ...
        for x in datespan(window_start_date,window_end_date+timedelta(weeks=1),delta=timedelta(weeks=1)):
            next_weekday_names.append(x.strftime("%A"))
...
def run_predictions_LSTM(model_id, model, window_start_date, window_end_date, target, narrative_list=[], features_local=[],
                    features_global=[], time_in=0, time_out=0, daily_pred=True):
    '''
    Description: Run predictions for LSTM models
    ...
    if daily_pred==False:
        sim_days = int(round(sim_days/7,0))
    ...
    for Tnarrative in narrative_list:
        ...
        if daily_pred:
            ...
            for x in datespan(window_start_date,window_end_date,delta=timedelta(weeks=time_out)):
        ...
        narrative_sim_data.setdefault(Tnarrative_,y_hat)
        narrative_gt_data.setdefault(Tnarrative_,sim_y) 
